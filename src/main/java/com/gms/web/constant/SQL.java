...
public class SQL {
	...
	public static final String MEMBER_FINDBYID=String.format("SELECT * FROM %s WHERE %s like ?", Database.TABLE_STUDENT,Database.MEMBER_ID);
	...
	"select t.* from (select @rownum:=@rownum+1 num,s.* from student s,(select @rownum:=0) r) t where num between ? and ?";
